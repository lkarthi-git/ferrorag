...
/// Represents a node within the execution graph.
/// 
/// Types implementing this trait define the core logic of a single step
/// and determine which node should be executed next.
#[async_trait]
pub trait Topology: Clone + PartialEq + Eq + std::fmt::Debug + Send + Sync + Sized {
    /// The global state shared across graph execution.
    type State;
    /// The state transition/mutation output produced by executing this node.
    type Delta;
    /// The error type returned during node execution.
    type Error: std::fmt::Debug + Send + Sync;

    /// Executes the core logic for this node given the current state.
    /// 
    /// Returns a `Delta` which will subsequently be applied to the state via the `Reducer`.
    ...
    /// 
    /// Returns `None` to indicate that graph execution is complete.
    fn route(&self, state: &Self::State) -> Option<Self>;
}

/// Applies state transitions generated by graph nodes.
pub trait Reducer<D, E> {
    /// Mutates the state using the provided delta.
    fn reduce(&mut self, delta: D) -> Result<(), E>;
}

/// Represents the final outcome of a graph execution run.
pub enum ExecutionResult<T: Topology> {
    /// The graph executed to completion (no next node was routed).
    Completed { state: T::State, ctx: ExecutionContext<T> },
    /// The graph execution was intentionally paused before executing a specified node.
    Suspended { state: T::State, pending_node: T , ctx: ExecutionContext<T> },
}

/// Errors that can occur during the execution of the graph.
#[derive(Debug)]
pub enum GraphError<E> {
    /// An error occurred during `Topology::execute_node`.
    NodeError(E),
    /// An error occurred while applying the delta to the state via `Reducer::reduce`.
    LogicalError(E),
    /// The execution exceeded the maximum permitted steps.
    MaxStepsExceeded(usize),
    /// The execution was externally cancelled via a `CancellationToken`.
    Cancelled,
    /// An error occurred while saving or loading state checkpoints.
    CheckpointError(String),
}

/// Provides a mechanism to persist and restore execution state.
#[async_trait]
pub trait Checkpoint<T: Topology> {
    /// Persists the current execution context and state.
    async fn save_checkpoint(&self, ctx: &ExecutionContext<T>, state: &T::State) -> Result<(), String>;
    
    /// Loads a previously saved execution context and state using the thread identifier.
    async fn load_checkpoint(&self, thread_id: &str) -> Result<Option<(ExecutionContext<T>, T::State)>, String>;
}

/// Dictates the frequency at which the graph engine saves its state.
pub enum CheckpointStrategy {
    /// Only save when the graph encounters a suspension point.
    OnSuspendOnly,            
    /// Save after a specific number of nodes have been executed.
    EveryNSteps(usize),       
    /// Save after every single node execution.
    Always,                 
}

/// Tracks the metadata, history, and routing information for a single graph execution thread.
#[derive(Debug, Clone)]
pub struct ExecutionContext<K> {
    /// A unique identifier for this execution run.
    pub thread_id: String,
    /// The next node scheduled for execution.
    pub next_node: Option<K>,
    /// The total number of nodes executed so far.
    pub step_count: usize,
    /// Custom key-value data associated with this execution.
    pub metadata: HashMap<String, String>,
    /// Internal flag tracking whether the current run was just resumed from a suspended state.
    #[doc(hidden)] 
    pub is_resuming: bool
}

impl<K> ExecutionContext<K> {
    /// Creates a new execution context starting at the specified node.
    ...
    /// Builder method to inject initial metadata into the context.
...
/// The core runtime engine for executing a topology-defined state graph.
...
impl<T: Topology> GraphEngine<T> {
    /// Initializes a new, unconfigured `GraphEngine`.
    pub fn new() -> Self {
        Self {
            max_steps: usize::MAX,
    ...
    /// Sets the maximum number of node executions permitted to prevent infinite loops.
    pub fn with_max_steps(mut self, steps: usize) -> Self {
        self.max_steps = steps;
        self
    }

    /// Specifies a node that, when reached, will pause execution and yield a `Suspended` result.
    pub fn with_interrupt_before(mut self, node: T) -> Self {
        self.interrupt_before.push(node);
        self
    }

    /// Attaches a cancellation token that can externally abort the graph execution.
    pub fn with_cancel_token(mut self, token: CancellationToken) -> Self {
        self.cancel_token = Some(token);
        self
    }

    /// Configures the checkpointing behavior for state persistence.
    ...
    /// Evaluates the `CheckpointStrategy` and optionally saves the current state.
    ...
    /// Unconditionally forces a state checkpoint save, bypassing the configured strategy.
    ...
    /// Resumes a previously suspended or interrupted graph execution.
    /// 
    /// The engine will look up the `thread_id` in the configured `Checkpoint` store.
    ...
    /// Starts or continues the continuous evaluation of the state graph.
    /// 
    /// Execution continues until the graph routes to `None`, hits a configured interrupt point, 
    /// exceeds the maximum step count, encounters an error, or is cancelled.
